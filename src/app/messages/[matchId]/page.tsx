...
export default async function ChatPage({
  ...
  const { data: match } = await supabase
    .from("matches")
    .select(
      "id, user_a_id, user_b_id, watch_a:watches!matches_watch_a_id_fkey(brand,model,photos), watch_b:watches!matches_watch_b_id_fkey(brand,model,photos), user_a:profiles!matches_user_a_id_fkey(username,avatar_url), user_b:profiles!matches_user_b_id_fkey(username,avatar_url)"
    )
    .eq("id", matchId)
    .maybeSingle();
