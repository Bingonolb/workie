...
export async function POST() {
  ...
  try {
    ...
    await stripe.subscriptions.update(company.stripe_subscription_id, { cancel_at_period_end: true });

    await supabase.from("companies").update({ subscription_cancel_at_period_end: true }).eq("id", profile.claimed_company_id);

    return NextResponse.redirect(`${baseUrl}/business/dashboard/subscription?canceled=1`, 303);
  } catch (e) {
