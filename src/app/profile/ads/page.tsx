      ...
      <main className="page-main-md" style={{ paddingTop: 24, paddingBottom: 48 }}>
        ...
        {paymentCanceled && (
          <div role="alert" style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 12, padding: "14px 18px", color: "#ef4444", fontSize: 14, fontWeight: 600, marginBottom: 20, display: "flex", alignItems: "center", gap: 10 }}>
            <XCircle size={16} aria-hidden="true" /> Paiement annulé. Complétez le paiement depuis la liste ci-dessous.
          </div>
        )}
        {error && (
              ...
              {filtered.map((c: AdCampaign) => {
                ...
                return (
                  <div key={c.id} style={{ position: "relative", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, overflow: "hidden" }}>
                    <Link href={`/profile/ads/${c.id}`} style={{ position: "absolute", inset: 0, zIndex: 0 }} aria-label={c.headline} />
                    ...
                    {/* Action bar */}
                    <div style={{ borderTop: "1px solid var(--border)", padding: "10px 16px", display: "flex", alignItems: "center", gap: 8, position: "relative", zIndex: 1 }}>
                      ...
                      <Link
                        ...
                        style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 12, fontWeight: 700, color: "var(--text-muted)", textDecoration: "none", padding: "5px 12px", borderRadius: 8, border: "1px solid var(--border2)" }}
                      >
                      ...
                      <span style={{ marginLeft: "auto", display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                        ...
                        {(() => {
                          ...
                          return (
                            <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 50, background: urgent ? "rgba(239,68,68,0.1)" : "rgba(16,185,129,0.08)", color: urgent ? "#ef4444" : "#10b981" }}>
                              {label}
                            </span>
                          );
                        })()}
