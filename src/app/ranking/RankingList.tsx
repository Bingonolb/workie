...
function RankingRow({ c, displayRank, isTop3, isLoggedIn, isBusiness, initialFav, maxScore }: {
  c: Company; displayRank: number; isTop3: boolean;
  isLoggedIn: boolean; isBusiness: boolean; initialFav: boolean; maxScore: number;
}) {
  const [fav, setFav] = useState(initialFav);
  const [score, setScore] = useState(Number(c.score ?? 0));
  ...
  const ratingPts = avgRating > 0 && reviewCount > 0 ? Math.round(avgRating * 20 * Math.log(reviewCount + 1)) : 0;
  ...
  const handleFav = (e: React.MouseEvent) => {
    ...
    const next = !fav;
    setFav(next);
    setScore(s => next ? s + 1 : Math.max(0, s - 1));
    startTransition(async () => {
      try { await toggleFavorite(c.id); }
      catch { setFav(prev); setScore(prevScore); }
    });
  };

  return (
    <Link href={`/company/${c.id}`} className="ranking-row"
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, justifyContent: "flex-end" }}>
        {!isBusiness && (
          <button
            onClick={handleFav}
            title={fav ? "Retirer la flamme" : "Flamme (+1 pt)"}
            style={{
              display: "flex", alignItems: "center", gap: 4,
              padding: "5px 9px", borderRadius: 8, cursor: "pointer",
              border: fav ? "1px solid rgba(249,115,22,0.5)" : "1px solid var(--border)",
              background: fav ? "rgba(249,115,22,0.1)" : "transparent",
              color: fav ? "#f97316" : "var(--text-muted)",
              transition: "all 0.15s", flexShrink: 0,
            }}
          >
            <Flame size={12} fill={fav ? "#f97316" : "none"} color={fav ? "#f97316" : "currentColor"} />
          </button>
        )}
...
export function RankingTable({ companies, isLoggedIn = false, isBusiness = false, favIds = [] }: { companies: Company[]; isLoggedIn?: boolean; isBusiness?: boolean; favIds?: string[] }) {
        ...
        {filtered.map((c, filteredIdx) => {
          ...
          return (
            <RankingRow
              ...
              isTop3={displayRank < 3}
              isLoggedIn={isLoggedIn}
              isBusiness={isBusiness}
              initialFav={favIds.includes(c.id)}
              maxScore={maxScore}
            />
          );
        })}
